...
model{
target += skew_normal_lpdf(alpha |", experts[i,1], ",", experts[i,2],",",(-1/experts[i,3]), ");
...
# define benchmarks
benchmarks <- c("uniform_lpdf(alpha | 0, 5)", "uniform_lpdf(alpha | -10,10)",
                paste0("normal_lpdf(alpha | 0,sqrt(10))"),
                paste0("normal_lpdf(alpha | 0,sqrt(100))"),
                paste0("normal_lpdf(alpha | 0,sqrt(1000))"))
...
for(j in 1:5){
  bf.exp.bench <- c()
  for(i in 1:4){
    bf.exp.bench[i] <-  bridgesampling::bf(bridge.benchmarks[[j]],
                                           bridge.experts[[i]])$bf
  }
  print(bf.exp.bench)
} # for comparison with all benchmarks
...
tmp.mtrx <- matrix(NA, ncol=4,nrow = 4)
for(i in 1:4){
  for(j in 1:4){
tmp.mtrx[i,j] <-    print(bridgesampling::bf(bridge.experts[[i]],bridge.experts[[j]])$bf)
  }
}
round(tmp.mtrx,2)
